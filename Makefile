...
build: $(VENV_PYTHON)
	$(VENV_PYTHON) -m pip install --upgrade pip > $(NULL) 2>&1
	$(VENV_PIP) install -e .
	$(VENV_PYTHON) -m PyInstaller --onefile --name=$(APP_NAME)_$(shell $(PYTHON) -c "import datetime;print(datetime.datetime.now().strftime('%Y%m%d%H%M%S'))") -c src/$(APP_NAME)/__main__.py
